...
###
#Standardizing the features (by subtracting the mean and dividing by the standard deviation)
#ensures that the learning process isn’t influenced by the inherent scale of the data.
#For example, the radius value might be much smaller than the perimeter value,
#and this difference in scale can affect how the model learns.
#This process helps to prevent such issues.
...
def initialize_weights(input_dim, output_dim, init_type="uniform"):
    if init_type=="uniform": weights = np.random.uniform(-1, 1, (input_dim, output_dim))
    elif init_type=="normal":weights = np.random.normal(0, 1, (input_dim, output_dim))
    elif init_type == "random":
        weights = np.random.random((input_dim, output_dim)) * 2 - 1  
        #-1 to 1
    else:
...
# plot the loss curves for initializations
plt.figure(figsize=(12, 6))
for lr in Learning_Rates:
    for init_type in initializations:
        for result in results:
